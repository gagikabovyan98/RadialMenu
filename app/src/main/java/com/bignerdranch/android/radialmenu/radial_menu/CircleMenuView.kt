...
import com.bignerdranch.android.radialmenu.R
...
class CircleMenuView : FrameLayout {
    ...
    private val mButtons: MutableList<View> = ArrayList()
    private val mButtonRect = Rect()
    private lateinit var mMenuButton: FloatingActionButton
    private lateinit var mRingView: RingEffectView
    ...
    class EventListener {
         ...
         */
        fun onMenuOpenAnimationStart(view: CircleMenuView) {}

        /**
         * Invoked on menu button click, after animation end.
         * @param view - current CircleMenuView instance.
         */
        fun onMenuOpenAnimationEnd(view: CircleMenuView) {}

        /**
         * Invoked on close menu button click, before animation start.
         * @param view - current CircleMenuView instance.
         */
        fun onMenuCloseAnimationStart(view: CircleMenuView) {}

        /**
         * Invoked on close menu button click, after animation end.
         * @param view - current CircleMenuView instance.
         */
        fun onMenuCloseAnimationEnd(view: CircleMenuView) {}
         ...
         */
        fun onButtonClickAnimationStart(view: CircleMenuView, buttonIndex: Int) {}
         ...
         * @param buttonIndex - clicked button zero-based index.
         */
        fun onButtonClickAnimationEnd(view: CircleMenuView, buttonIndex: Int) {}
         ...
         */
        fun onButtonLongClick(view: CircleMenuView, buttonIndex: Int): Boolean {
            return false
        }
         ...
         * @param buttonIndex - clicked button zero-based index.
         */
        fun onButtonLongClickAnimationStart(view: CircleMenuView, buttonIndex: Int) {}
         ...
         * @param buttonIndex - clicked button zero-based index.
         */
        fun onButtonLongClickAnimationEnd(view: CircleMenuView, buttonIndex: Int) {}
    }

    private inner class OnButtonClickListener : OnClickListener {
        override fun onClick(view: View) {
            if (mIsAnimating) {
                return
            }
            val click = getButtonClickAnimation(view as FloatingActionButton)
            click.setDuration(durationRing.toLong())
            click.addListener(object : AnimatorListenerAdapter() {
                override fun onAnimationStart(animation: Animator) {
                    eventListener?.onButtonClickAnimationStart(
                        this@CircleMenuView,
                ...
                override fun onAnimationEnd(animation: Animator) {
                    mClosedState = true
                    eventListener?.onButtonClickAnimationEnd(
                        this@CircleMenuView,
                        mButtons.indexOf(view)
                    )
                }
    ...
    private inner class OnButtonLongClickListener : OnLongClickListener {
        override fun onLongClick(view: View): Boolean {
            val result =
                eventListener?.onButtonLongClick(this@CircleMenuView, mButtons.indexOf(view))
                    ?: false
            if (result && !mIsAnimating) {
                val click = getButtonClickAnimation(view as FloatingActionButton)
                click.setDuration(longClickDurationRing.toLong())
                click.addListener(object : AnimatorListenerAdapter() {
                    override fun onAnimationStart(animation: Animator) {
                        eventListener?.onButtonLongClickAnimationStart(
                            this@CircleMenuView,
                            mButtons.indexOf(view)
                        )
                    }
                    ...
                    override fun onAnimationEnd(animation: Animator) {
                        mClosedState = true
                        eventListener?.onButtonLongClickAnimationEnd(
                            this@CircleMenuView,
                            mButtons.indexOf(view)
                        )
                        eventListener?.onButtonLongClickAnimationEnd(
    ...
    constructor(context: Context, attrs: AttributeSet?, @AttrRes defStyleAttr: Int = 0) : super(
        ...
        requireNotNull(attrs) { "No buttons icons or colors set" }
        ...
        try {
            ...
            iconMenu =
                a.getResourceId(R.styleable.CircleMenuView_icon_menu, R.drawable.ic_menu_black_24dp)
            iconClose = a.getResourceId(
                R.styleable.CircleMenuView_icon_close,
                R.drawable.ic_close_black_24dp
            )
            durationRing = a.getInteger(
                R.styleable.CircleMenuView_duration_ring,
                resources.getInteger(android.R.integer.config_mediumAnimTime)
            )
            longClickDurationRing = a.getInteger(
                R.styleable.CircleMenuView_long_click_duration_ring,
                resources.getInteger(android.R.integer.config_longAnimTime)
            )
            durationOpen = a.getInteger(
                R.styleable.CircleMenuView_duration_open,
                resources.getInteger(android.R.integer.config_mediumAnimTime)
            )
            durationClose = a.getInteger(
    ...
    override fun onLayout(changed: Boolean, left: Int, top: Int, right: Int, bottom: Int) {
        ...
        val lp = mRingView.layoutParams as LayoutParams
        lp.width = right - left
    ...
    private fun initLayout(context: Context) {
        ...
        mRingView = findViewById<RingEffectView>(R.id.ring_view)
    }

    private fun initMenu(menuButtonColor: Int) {
        val animListener: AnimatorListenerAdapter = object : AnimatorListenerAdapter() {
            override fun onAnimationStart(animation: Animator) {
                if (mClosedState) {
        ...
        mMenuButton.setOnClickListener(object : OnClickListener {
            override fun onClick(view: View) {
                if (mIsAnimating) {
                    return
                }
                val animation: Animator =
                    if (mClosedState) openMenuAnimation else closeMenuAnimation
                animation.setDuration((if (mClosedState) durationClose else durationOpen).toLong())
    ...
    private fun initButtons(context: Context, icons: List<Int>, colors: List<Int>) {
        val buttonsCount = min(icons.size, colors.size)
        for (i in 0 until buttonsCount) {
    ...
    private fun offsetAndScaleButtons(
        centerX: Float,
        centerY: Float,
        angleStep: Float,
        offset: Float,
    ...
    private val openMenuAnimation: Animator
        private get() {
            val alphaAnimation =
                ObjectAnimator.ofFloat(mMenuButton, "alpha", DEFAULT_CLOSE_ICON_ALPHA)
            ...
            rotateAnimation.addUpdateListener(object : AnimatorUpdateListener {
    ...
    private fun openOrClose(open: Boolean, animate: Boolean) {
        ...
        if (animate) {
            ...
            val visibility = if (open) VISIBLE else INVISIBLE
            for (view in mButtons) {
    ...
    companion object {
        ...
        private const val DEFAULT_DISTANCE = DEFAULT_BUTTON_SIZE * 1.5f
        private const val DEFAULT_RING_SCALE_RATIO = 1.3f
        private const val DEFAULT_CLOSE_ICON_ALPHA = 0.3f
    }
}
